...
@fragment fn fs_main(
  ...
  var nrIterations = transform.params[0][0];
  var radiusSize = transform.params[1][0];
  ...
  for (var gen = 0; gen < numGenerations; gen++) {
    ...
    let d2 = createLeafPattern(rotatedSt1 * 0.6, 6.0, time * 1.2, expansion2, 0.8 * multiplier);
    let leaf2 = smoothstep(0.28, 0.3, d2) * smoothstep(0.5, 0.4,d2) * opacity2;
    ...
    let d3 = createLeafPattern(rotatedSt2 * 0.7, 8.0, time * 0.2, expansion3, 0.8 * multiplier);
    let leaf3 = smoothstep(0.15, 0.2, d3) * smoothstep(0.8, 0.6,d3) * opacity3;
    ...
    let d4 = createLeafPattern(rotatedSt3 * 0.6, 5.0, time * 2.8, expansion4, 0.67 * multiplier);
    let leaf4 = smoothstep(0.2,0.3, d4) * smoothstep(0.5, 0.2,d4) * opacity4;
    ...
    let d5 = createLeafPattern(rotatedSt4 * 0.5, 3.0, time * 1.0, expansion5, 0.5 * multiplier);
    let leaf5 = smoothstep(0.0,0.2, d5) * smoothstep(0.5, 0.3, d5) * opacity5;

    // Layer 6
    let rotatedSt5 = rotate2D(st, time * 0.3 + genOffset);
    let expansion6 = getContinuousExpansion(time * 1.8, genOffset + 0.3);
    let opacity6 = getFadeOpacity(time * 1.2, genOffset + 0.3);
    let d6 = createLeafPattern(rotatedSt5 * 0.3, 4.0, time * 0.1, expansion6, 0.5 * multiplier);
    // let leaf6 = smoothstep(0.1,1.5, d6) * step(d6, 0.5) * opacity6;
    let leaf6 = step(0.5, d6) * step(d6, 0.5) * opacity6;
    ...
    finalColor += leaf6 * amber * generationDimming;
  }
